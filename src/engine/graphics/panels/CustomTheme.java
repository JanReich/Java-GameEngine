...
public class CustomTheme extends DefaultMetalTheme {
  ...
  private void loadProperties(File file) {
    ...
    temp = FileHelper.getProperty(file, "titlefont");
      if (null != temp) {
          windowTitleFont = parseFont(temp);
      }
    temp = FileHelper.getProperty(file, "controlfont");
      if (null != temp) {
          controlFont = parseFont(temp);
      }
  }

  private ColorUIResource parseColor(final String color) {
    int r, g, b;
    r = g = b = 0;
    try {
  ...
  private FontUIResource parseFont(final String font) {
